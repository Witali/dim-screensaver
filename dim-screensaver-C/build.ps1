...
try {
    ...
    $compile = "cl.exe /nologo /W4 /std:c17 /TC /DUNICODE /D_UNICODE $optimization /Fo:publish\ dim_screensaver.c /link /SUBSYSTEM:WINDOWS /OUT:publish\DimScreensaverC.exe user32.lib gdi32.lib shell32.lib"
    $manifest = "mt.exe -nologo -manifest app.manifest -outputresource:publish\DimScreensaverC.exe;#1"
    ...
    Copy-Item .\publish\DimScreensaverC.exe .\publish\DimScreensaverC.scr -Force
    Write-Host "Built .\publish\DimScreensaverC.scr"
}
finally {
    Pop-Location
}
